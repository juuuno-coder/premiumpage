...
import { DB, CATEGORY_INFO, BRANDS, VAISALA_APPLICATIONS, SETRA_APPLICATIONS_SENSOR, SETRA_APPLICATIONS_SOLUTION, JUMO_CATEGORY_INTRO, JUMO_APPLICATIONS } from './data'
...
function JumoApplicationModal({ appKey, onClose }: { appKey: string; onClose: () => void }) {
    useEffect(() => {
        const handler = (e: KeyboardEvent) => { if (e.key === 'Escape') onClose() }
        window.addEventListener('keydown', handler)
        return () => window.removeEventListener('keydown', handler)
    ...
    const appData = JUMO_APPLICATIONS[appKey]
    if (!appData) return null
    ...
    return (
        ...
        >
            ...
            >
                {/* Header */}
                <div className="flex items-start justify-between px-6 md:px-10 py-5 border-b border-neutral-100 shrink-0 bg-gradient-to-r from-orange-50 to-white">
                    <div>
                        <p className="text-[10px] text-orange-600 font-black uppercase tracking-[0.3em] mb-1">JUMO Applications & Solutions</p>
                        <h2 className="text-2xl md:text-4xl font-black text-neutral-900 tracking-tight">{appData.title}</h2>
                        <p className="text-sm text-neutral-500 mt-2">{appData.shortDesc}</p>
                    </div>
                    <button onClick={onClose} className="p-2 hover:bg-white rounded-lg transition-colors ml-6 shrink-0 mt-1">
                ...
                <div className="px-6 md:px-10 py-8">
                    ...
                    {appData.applications && appData.applications.length > 0 && (
                        <div className="mb-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                {appData.applications.map((app: string, i: number) => (
                                    <div key={i} className="p-4 border border-orange-100 rounded-lg bg-orange-50/50">
                                        <p className="text-sm font-bold text-neutral-900 flex items-start gap-2">
                    ...
                    {appData.measurementProducts && appData.measurementProducts.length > 0 && (
                        <div className="mb-8">
                            <h3 className="text-lg md:text-xl font-black text-neutral-900 mb-4 flex items-center gap-2">
                                <span className="w-1.5 h-1.5 bg-orange-600 inline-block"></span>
                                Measurement Products
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                {appData.measurementProducts.map((product: string, i: number) => (
...
function BrandPage({
    ...
    const [selectedJumoKey, setSelectedJumoKey] = useState<string | null>(null)
    ...
    const appTitleToKey: Record<string, string> = {
        ...
        'Semiconductor & Display': 'semiconductor',
    ...
    return (
        <CatalogPage currentTab={tab}>
            ...
            {/* JUMO Application Modal */}
            <AnimatePresence>
                {selectedJumoKey && (
                    <JumoApplicationModal
                        appKey={selectedJumoKey}
                        onClose={() => setSelectedJumoKey(null)}
                    />
                )}
            </AnimatePresence>
                    ...
                    sectionIdx === i && (
                        <div key={i} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            {sec.items.map((item, j) => {
                                ...
                                const isJumo = JUMO_APPLICATIONS[appKey] !== undefined
                                const handleClick = () => {
                                    if (isSetraSensor) {
